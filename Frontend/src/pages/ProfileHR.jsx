...
const ProfileHR = () => {
    ...
    useEffect(() => {
        if (!headers.id || !headers.authorization) {
            ...
            const fetchProfile = async () => {
                try {
                    const response = await axios.get(`http://localhost:3007/api/v1/gethrinfo`, { headers });
    ...
    useEffect(() => {
        if (!headers.id || !headers.authorization) {
            ...
            const fetchInternInfo = async () => {
                try {
                    const response = await axios.get(`http://localhost:3007/api/v1/hrIntern-info`, { headers });
    ...
    const deleteIntern = async (internId) => {
        ...
        if (confirmed) {
            try {
                await axios.delete(`http://localhost:3007/api/v1/delete-intern/${internId}`, { headers });
